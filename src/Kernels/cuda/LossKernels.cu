...
//         // Fill with zeros for invalid targets  
//         for (int64_t j = 0; j < vocab_size; ++j) {  
//             grad[i * vocab_size + j] = static_cast<T>(0);  
//         }  
//         return;  
//     }  
  
//     // Use shared memory for better performance with large vocab  
//     extern __shared__ float sdata[];  
//     float* s_logits = sdata;  
//     float* s_exp_vals = sdata + vocab_size;  
      
//     // Load logits to shared memory  
//     for (int64_t j = threadIdx.x; j < vocab_size; j += blockDim.x) {  
//         s_logits[j] = static_cast<float>(logits[i * vocab_size + j]);  
//     }  
//     __syncthreads();  
      
//     // Find max value  
//     float max_v = s_logits[0];  
//     for (int64_t j = 1; j < vocab_size; ++j) {  
//         max_v = fmaxf(max_v, s_logits[j]);  
//     }  
      
//     // Compute exp values and sum  
//     float sum_exp = 0.0f;  
//     for (int64_t j = 0; j < vocab_size; ++j) {  
//         float val = expf(s_logits[j] - max_v);  
//         s_exp_vals[j] = val;  
//         sum_exp += val;  
//     }  
...
//     // Allocate shared memory for logits and exp values  
//     size_t smem_size = 2 * vocab_size * sizeof(float);  
...
//         // Fallback to register-based version for very large vocab  
//         printf("Warning: Shared memory too large, using register fallback\n");  
//         sparse_ce_backward_kernel_typed<T, T_idx><<<blocks, threads, 0, stream>>>(  
//             logits, targets, grad_logits, batch_size, vocab_size, scale  
//         );  
//     } else {  
//         // Use shared memory version  
//         sparse_ce_backward_kernel_typed<T, T_idx><<<blocks, threads, smem_size, stream>>>(  
//             logits, targets, grad_logits, batch_size, vocab_size, scale  
//         );  
//     }  
...
//     cudaStream_t stream  
// ) {  
//     sparse_cross_entropy_backward_cuda_impl<T, T_idx>(  
//         logits, targets, grad_logits, batch_size, vocab_size, scale, stream  
//     );  
// }  
